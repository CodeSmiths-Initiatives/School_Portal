...
function StatusPanel({
  ...
  const config = {
    preparing: {
      ...
      className: "border-[#d6e2f3] bg-[#f5f8fd] text-[#36507d]",
      iconClassName: "animate-spin",
    },
    verifying: {
      icon: ShieldCheck,
      title: "Verifying payment",
      className: "border-[#d8ebda] bg-[#f4fbf5] text-[#2f6a39]",
      iconClassName: "",
    },
    cancelled: {
      icon: CircleAlert,
      title: "Payment cancelled",
      className: "border-[#f2dfb5] bg-[#fff9ea] text-[#8a6112]",
      iconClassName: "",
    },
    failed: {
      icon: CircleAlert,
      title: "Payment needs attention",
      className: "border-[#f0d1d7] bg-[#fff7f8] text-[#9a2c46]",
      iconClassName: "",
    },
  }[status];
...
export default function Payment({
  ...
  const { status, message, reset, startPayment } =
    useAdmissionPaystackPayment();
  ...
  async function handlePayment() {
    ...
    const validation = paymentIntentSchema.safeParse({
      ...
      collegeSlug,
      applicationId: application?.persisted ? application.id : undefined,
    ...
    await startPayment({
      ...
      collegeSlug: validation.data.collegeSlug,
      applicationId: validation.data.applicationId,
      applicationNumber: validation.data.applicationNumber,
    ...
    <div className="surface-card mx-auto w-full max-w-2xl p-5 sm:p-6 lg:p-8">
      ...
      <div className="mb-6 grid items-stretch gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(16rem,0.8fr)]">
        ...
        <div className="flex h-full flex-col rounded-2xl border border-[#d8e3f0] bg-white p-5 shadow-[0_14px_32px_-28px_rgba(23,48,95,0.45)]">
          ...
          {/* {collegeName && (
						<div className="mt-4 rounded-lg border border-[#dce6f2] bg-[#f8fbff] px-3 py-2">
							<p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8a9ab5]">
								College
							</p>
							<p className="mt-1 text-xs font-bold text-[#17305f]">
								{collegeName}
							</p>
						</div>
					)} */}
          {/* {application?.applicationNumber && (
						<div className="mt-3 rounded-lg border border-[#e5edf6] bg-white px-3 py-2">
							<p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8a9ab5]">
								Application
							</p>
							<p className="mt-1 break-all text-xs font-bold text-[#17305f]">
								{application.applicationNumber}
							</p>
						</div>
					)} */}
      ...
      >
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-[#4f7757]">
                  Checkout opens in Paystack and the application only moves
                  forward after our server confirms the transaction reference
                  and amount.
                </p>
              </div>
            </div>
        ...
        {(status !== "idle" || localError) && (
          <div className="space-y-3">
            ...
            {localError && <FieldFeedback message={localError} />}
          </div>
        )}
        ...
        <p className="mx-auto max-w-xl text-center text-xs leading-relaxed text-[#8193af]">
          Powered by Paystack test checkout for MVP review. Webhook
          reconciliation can be added next for production-grade reliability.
        </p>
      </form>
    </div>
  );
