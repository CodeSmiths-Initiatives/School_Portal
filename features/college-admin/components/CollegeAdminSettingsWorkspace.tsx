...
export default function CollegeAdminSettingsWorkspace({
  ...
  const loadNotices = useCallback(async () => {
    ...
    try {
      ...
      setNotices(
        (payload as AppNotificationListPayload).notifications.map(
          (notification) =>
  ...
  const filteredNotices = useMemo(() => {
    ...
    return notices.filter((notice) => {
      const haystack =
        `${notice.title} ${notice.message} ${notice.createdBy}`.toLowerCase();
  ...
  function updateForm<Key extends keyof NoticeForm>(
    key: Key,
  ...
  async function patchNotice(
    ...
    try {
      const response = await fetch(
        `/api/notifications/${encodeURIComponent(noticeId)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      <div className="rounded-3xl border border-[#d7e2f0] bg-white p-4 shadow-[0_18px_45px_rgba(13,43,85,0.08)] sm:p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          ...
          <select
            value={status}
            onChange={(event) =>
              setStatus(event.target.value as NoticeStatus | "all")
            }
          ...
          <select
            value={audience}
            onChange={(event) =>
              setAudience(event.target.value as NoticeAudience | "all")
            }
            className="h-12 rounded-2xl border border-[#d3dfed] bg-[#f8fbff] px-4 text-sm font-bold text-[#0D2B55] outline-none focus:border-[#2E86C1]"
          >
          ...
          <select
            value={severity}
            onChange={(event) =>
              setSeverity(event.target.value as NoticeSeverity | "all")
            }
            className="h-12 rounded-2xl border border-[#d3dfed] bg-[#f8fbff] px-4 text-sm font-bold text-[#0D2B55] outline-none focus:border-[#2E86C1]"
          >
                      ...
                      <RowActionMenu
                        ...
                        items={[
                          ...
                          },
                          {
                            ...
                            disabled:
                              notice.status === "active" || !canUpdateSettings,
                            onSelect: () =>
                              patchNotice(
                                notice.id,
                                { status: "active" },
                          ...
                          },
                          {
                            ...
                            disabled:
                              notice.status === "expired" || !canUpdateSettings,
