...
import {
	...
	type CreateHostelComplaintInput,
...
export async function POST(request: Request) {
	try {
		...
		if (!collegeSlug || session.user.collegeSlug !== collegeSlug) {
			return NextResponse.json(
				{ error: "This complaint is outside your college scope." },
				{ status: 403 },
			);
		}
		...
		if (!hasPermissions(permissions, ["hostels.view"], { mode: "any" })) {
			return NextResponse.json(
				{ error: "You do not have permission to raise hostel complaints." },
				{ status: 403 },
			);
		}

		const payload = (await request.json()) as CreateHostelComplaintInput;
		const result = await createHostelComplaint(collegeSlug, payload);
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error
						? error.message
						: "Unable to create hostel complaint.",
			},
			{ status: 400 },
		);
	}
