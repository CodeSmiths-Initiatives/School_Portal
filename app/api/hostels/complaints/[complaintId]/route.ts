...
import {
	...
	type UpdateHostelComplaintInput,
...
export async function PATCH(
	...
	try {
		...
		if (!collegeSlug || session.user.collegeSlug !== collegeSlug) {
			return NextResponse.json(
				{ error: "This complaint request is outside your college scope." },
				{ status: 403 },
			);
		}
		...
		if (!hasPermissions(permissions, ["hostels.update"], { mode: "any" })) {
			return NextResponse.json(
				{ error: "You do not have permission to manage hostel complaints." },
				{ status: 403 },
			);
		}

		const { complaintId } = await ctx.params;
		const payload = (await request.json()) as UpdateHostelComplaintInput;
		const result = await updateHostelComplaint(collegeSlug, complaintId, payload);
