...
import { updateHostelBed } from "@/lib/services/hostel.service";
import {
	getDefaultPermissionsForDomain,
	hasPermissions,
	type UserPermissionKey,
} from "@/lib/rbac";
import { updateHostelBedSchema } from "@/lib/validation";
...
export async function PATCH(
	...
	try {
		...
		if (!collegeSlug || session.user.collegeSlug !== collegeSlug) {
			return NextResponse.json(
				{ error: "This bed update is outside your college scope." },
				{ status: 403 },
			);
		}
		...
		if (!hasPermissions(permissions, ["hostels.update"], { mode: "any" })) {
			return NextResponse.json(
				{ error: "You do not have permission to update hostel beds." },
				{ status: 403 },
			);
		}

		const { bedId } = await params;
		const parsed = updateHostelBedSchema.safeParse(await request.json());

		if (!parsed.success) {
			return NextResponse.json(
				{ error: parsed.error.issues[0]?.message ?? "Invalid bed details." },
				{ status: 400 },
			);
		}

		const result = await updateHostelBed(collegeSlug, bedId, parsed.data);

		return NextResponse.json(result);
	} catch (error) {
		return NextResponse.json(
			{
				error: error instanceof Error ? error.message : "Unable to update bed.",
			},
			{ status: 400 },
		);
	}
